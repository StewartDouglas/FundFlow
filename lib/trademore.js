...
module.exports = {
	...
	createrawtransaction: function(txid,address,amount, callback){
		...
		var addressAndAmount = '{"' + address + '": ' + amount + '}';
		var jsonAddAndAmnt = JSON.parse(addressAndAmount);

		client.createRawTransaction([{"txid": txid, "vout": 0}], jsonAddAndAmnt, function(err, rawtransaction){
	...
	signrawtransaction: function(rawtransaction, callback){
		client.signRawTransaction(rawtransaction, function(err, signedtransaction){
			...
			callback(signedtransaction.hex);
		});
	},

	sendrawtransaction: function(rawtransaction, callback){
		client.sendRawTransaction(rawtransaction,function(err,confirmation){
			if(err) { console.log('Error broadcasting transaction:' + err) }
			callback(confirmation);
		});
	},

	createContract: function(contractName,borrower,lender,mediator,numPayments,interest,day){
		...
		var fs = require('fs')
			...
			fs.readFile('./hash.txt', 'utf8', function (err,data) {
			  ...
			  var result = data.replace(/_BORROWER_/g, borrower);
			  	  result = data.replace(/_LENDER_/g, lender);
			  	  result = data.replace(/_MEDIATOR_/g, mediator);
			  	  result = data.replace(/_NUM_PAYMENTS_/g, numPayments);
			  	  result = data.replace(/_INTEREST_/g, interest);
			  	  result = data.replace(/_DAY_/g,day);
			  	  
			  var contract = './lib/contracts/' + contractName + '.txt';

			  fs.writeFile(contract, result, 'utf8', function (err) {
